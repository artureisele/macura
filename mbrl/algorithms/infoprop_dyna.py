...
def train(
    ...
    eval_traj_plot = plt.scatter(sac_buffer.obs[:sac_buffer.num_stored,0], sac_buffer.obs[:sac_buffer.num_stored,0], c=sac_buffer.info_lost_current_step[:sac_buffer.num_stored,1], cmap="viridis", s=5)
    """
    plt.axis([-2 * 2.4, 2 * 2.4, -2 * 24 * 2 * math.pi / 360, 2 * 24 * 2 * math.pi / 360])
    plt.colorbar(eval_traj_plot, label="Lost Total Info")
    plt.xlabel("X")
    plt.ylabel("Theta")
    plt.title("Lost Total")
    plt.show()
    """
    n_evals = 0
    sum_rewards = 0
